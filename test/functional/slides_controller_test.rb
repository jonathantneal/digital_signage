...
class SlidesControllerTest < ActionController::TestCase
  ...
  test "admin should create slide" do
    as_user(:admin) do
      assert_difference('Slide.count') do
        ...
        post :create, :slide => { :title => 'JUST_CREATED', :content => slide_file, :delay => 5, :color => "000000", :department_id => departments(:one).to_param}
      end
  ...
  test "manager one should create slide with department one" do
    as_user(:manager_one) do
      assert_difference('Slide.count') do
        slide_file = fixture_file_upload('files/slide1.png', 'image/png')
        post :create, :slide => { :title => 'JUST_CREATED_again', :content => slide_file, :delay => 5, :color => "000000", :department_id => departments(:one).to_param}
      end

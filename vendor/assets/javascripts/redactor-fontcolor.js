...
RedactorPlugins.fontcolor = {
  ...
  {
    var rule = 'color';
    if (name === 'backcolor') rule = 'text-shadow';
    ...
    var onSwatch = function(e)
    {
      ...
      swatch_rule = $this.data('rule');
      swatch_value = (rule == 'text-shadow') ? '3px 3px 10px '+$this.attr('rel') : $this.attr('rel');

      _self.pickerSet(swatch_rule, swatch_value);
